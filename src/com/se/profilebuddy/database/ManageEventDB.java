...
public class ManageEventDB
{
	...
	private static final String[] EVENT_PROJECTION = { EVENT_ID, FOREIGN_EVENT_ID, TITLE,
			START_TIME, END_TIME, DURATION, MODE, RECURSIVE, STATUS };
	...
	/**
	 * Method to get a cursor to the all the entries in the Event table
	 * 
	 * @return Cursor to the entire Events which are active.
	 */
	public Cursor getEventsCursor(boolean active)
	{
		Log.i(TAG, "Entering  getEventsCursor");
		...
		Log.i(TAG, "Exiting  getEventsCursor");
		return cursor;
	}
	...
	public List<CalendarEvent> fetchEvents(boolean active)
	{
		...
		while (cursor.moveToNext())
		{
			...
			event.setDuration(cursor.getLong(5));
			event.setMode(cursor.getInt(6));
			event.setRecursive(cursor.getInt(7) == 1 ? true : false);
			event.setStatus(cursor.getInt(8));
